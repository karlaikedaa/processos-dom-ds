...
import {
  ...
  FilePlus2, ExternalLink, RotateCcw, Settings, Check,
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-5 items-start">
      ...
      <div className="bg-white rounded-xl flex flex-col min-w-0 w-full xl:flex-1"
        ...
        <div className="p-4 overflow-y-auto" style={{ maxHeight: '520px', scrollbarWidth: 'thin' }}>
          {filteredDocs.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {filteredDocs.map(doc => (
                <div key={doc.id}
                  ...
                  style={{ background: selectedDocs.has(doc.id) ? 'rgba(220,10,10,0.03)' : 'white', border: '1px solid var(--border)' }}>
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <Button onClick={() => setDetailDoc(doc)} variant="ghost" size="sm" className="flex items-center gap-1.5">
        ...
        <div className="overflow-y-auto" style={{ maxHeight: '520px', scrollbarWidth: 'thin' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
            {filteredTarefas.map(item => (
              <div key={item.id}
                className="p-4 rounded-lg hover:shadow-md transition-all"
                style={{ background: selectedTarefas.has(item.id) ? 'rgba(254,166,1,0.03)' : 'white', border: '1px solid var(--border)' }}>
                <div className="flex items-start gap-2">
                  <div className="mt-0.5 shrink-0">
              ...
              <tr key={item.id}
                ...
                <td className="px-2.5 py-3">
                  <div className="flex items-center justify-end gap-1.5" style={{ flexWrap: 'nowrap' }}>
                    <Button onClick={() => setReenvioItem(item)} variant="secondary" size="sm" className="flex items-center gap-1.5">
                      <Upload size={14} /> Upload
                    </Button>
                    <Button onClick={() => setReenvioItem(item)} variant="default" size="sm" className="flex items-center gap-1.5">
                      <RefreshCw size={14} /> Reenviar
                    </Button>
...
export function StatusIntegracao({ onBack = NOOP }: StatusIntegracaoProps) {
            ...
            <TabsTrigger value="dominio">
              ...
              {errosDominio > 0 && (
                <span className="inline-flex items-center justify-center rounded-full px-1.5"
                  style={{ fontSize: '10px', fontWeight: 'var(--font-weight-semibold)', background: activeTab === 'dominio' ? 'var(--chart-4)' : 'rgba(220,10,10,0.15)', color: activeTab === 'dominio' ? 'white' : 'var(--chart-4)', minWidth: '20px', height: '18px' }}>
                  {errosDominio}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value="portal">
              Integrações Portal do Cliente
              {errosPortal > 0 && (
                <span className="inline-flex items-center justify-center rounded-full px-1.5"
                  style={{ fontSize: '10px', fontWeight: 'var(--font-weight-semibold)', background: activeTab === 'portal' ? 'var(--chart-4)' : 'rgba(220,10,10,0.15)', color: activeTab === 'portal' ? 'white' : 'var(--chart-4)', minWidth: '20px', height: '18px' }}>
                  {errosPortal}
                </span>
              )}
            </TabsTrigger>
