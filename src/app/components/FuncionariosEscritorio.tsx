...
import {
  Plus, Search, Edit2, Trash2, Check, X, ChevronDown,
...
function EmployeeDetail({ func, funcionarios, depts, allResponsabilidades, transferencias, menuPermissoes, onBack, onNavigateToConfig, onUpdate, onAddTransfer }: {
  func: Funcionario;
  funcionarios: Funcionario[];
  depts: Departamento[];
  allResponsabilidades: Record<string, Responsabilidade[]>;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-white px-4 md:px-6 shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        {/* Breadcrumb */}
...
export function FuncionariosEscritorio({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Page header */}
      <div className="bg-white px-4 md:px-6 shrink-0 flex items-center justify-between gap-4 flex-wrap"
        style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex-1 min-w-0">
          {/* Breadcrumb */}
          <div style={{ paddingTop: '24px', paddingBottom: '24px' }}>
