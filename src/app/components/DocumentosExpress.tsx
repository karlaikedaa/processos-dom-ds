...
import {
  ...
  ZapOff, Globe, FolderOpen, FileDown, Settings,
} from 'lucide-react';
...
function TabUploadExpress() {
  ...
  return (
    <div className="flex flex-col" style={{ gap: '24px' }}>
      {/* Flag informativa */}
      <div className="flex items-start gap-3 p-4 rounded-lg" style={{ background: 'rgba(33, 150, 243, 0.08)', border: '1px solid rgba(33, 150, 243, 0.2)' }}>
        <Info size={18} style={{ color: 'var(--chart-2)', flexShrink: 0, marginTop: '2px' }} />
        <div className="flex-1">
          <p style={{ fontSize: 'var(--text-label)', color: 'var(--foreground)', lineHeight: 1.5 }}>
            Se preferir, baixe a pasta Express no seu computador e envie automaticamente para o Express os arquivos salvos na sua máquina.
            <strong> Importante:</strong> só serão enviados os arquivos salvos na pasta Express.
          </p>
          <a href="#" className="inline-flex items-center gap-1.5 mt-2" style={{ fontSize: 'var(--text-label)', color: 'var(--chart-2)', textDecoration: 'none', fontWeight: 'var(--font-weight-semibold)' }}>
            <FolderOpen size={14} /> Baixar pasta Express
          </a>
        </div>
      </div>
      ...
      <WhiteBlock>
        <div
          ...
          style={{
            ...
            background: isDragging ? 'rgba(214,64,0,0.04)' : 'transparent',
            padding: '32px 24px',
            minHeight: '140px',
          }}
      ...
      <WhiteBlock>
        ...
        <div className="bg-white rounded-xl flex flex-col min-w-0 w-full xl:flex-1"
          ...
          <div className="p-4 overflow-y-auto" style={{ maxHeight: '480px', scrollbarWidth: 'thin' }}>
            {filteredNI.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {filteredNI.map(doc => (
                  <div key={doc.id}
                    ...
                    style={{ background: selectedNaoId.has(doc.id) ? 'rgba(220,10,10,0.03)' : 'white', border: '1px solid var(--border)' }}>
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <Button onClick={() => setDetailDoc(doc)} variant="secondary" size="sm" className="flex items-center gap-1.5">
                            <ExternalLink size={14} /> Exibir erro
                          </Button>
                          ...
                          <button onClick={() => setDeleteModalNI({ ids: [doc.id] })}
                            className="w-7 h-7 flex items-center justify-center rounded cursor-pointer hover:bg-muted transition-colors ml-auto shrink-0">
            ...
            {filteredI.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {filteredI.map(item => (
                  <div key={item.id}
                    className="p-4 rounded-lg hover:shadow-md transition-all"
                    style={{ background: selectedId.has(item.id) ? 'rgba(254,166,1,0.03)' : 'white', border: '1px solid var(--border)' }}>
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <Button variant="ghost" size="sm" className="flex items-center gap-1.5">
                            <ExternalLink size={14} /> Exibir documento
                          </Button>
                          <Button onClick={() => setActionItem(item)} variant="secondary" size="sm" className="flex items-center gap-1.5">
                            <Settings size={14} /> Gerenciar
                          </Button>
                          <Button onClick={() => setConfirmModal({ ids: [item.id] })} variant="default" size="sm" className="flex items-center gap-1.5">
...
interface DocumentosExpressProps {
  onBack?: () => void;
}

export function DocumentosExpress({ onBack = NOOP }: DocumentosExpressProps) {
  ...
  return (
    <div className="p-3 md:p-4 lg:p-6" style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f6f6f6' }}>

      {/* Seção 1: Título */}
      <div style={{ paddingBottom: '16px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'var(--font-weight-semibold)', color: 'var(--foreground)', marginBottom: '8px' }}>
          Documentos Express
        </h1>
        <p style={{ fontSize: 'var(--text-caption)', color: 'var(--muted-foreground)' }}>
      ...
      {/* Blocos 2 e 3: Upload/Filtros e Conteúdo (dentro de cada aba) */}
